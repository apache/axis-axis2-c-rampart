...
AXIS2_EXTERN saml_artifact_t* AXIS2_CALL saml_artifact_create(axutil_env_t *env)
{
...
AXIS2_EXTERN void AXIS2_CALL saml_artifact_free(saml_artifact_t *artifact, axutil_env_t *env)
{
...
AXIS2_EXTERN axis2_char_t* AXIS2_CALL saml_artifact_get_data(saml_artifact_t *artifact, axutil_env_t *env)
{
	if(artifact)
...
AXIS2_EXTERN int AXIS2_CALL saml_artifact_set_data(saml_artifact_t *artifact, axutil_env_t *env, axis2_char_t *data)
{
	if(artifact->artifact)
	{
		AXIS2_FREE(env->allocator, artifact->artifact);
...
AXIS2_EXTERN saml_request_t* AXIS2_CALL saml_request_create(axutil_env_t *env)
{
...
AXIS2_EXTERN void AXIS2_CALL saml_request_free(saml_request_t *request, axutil_env_t *env)
{
	int size = 0, i = 0;
	saml_artifact_t *artifact = NULL;
														 ...
														 axutil_env_t *env)
{
	...
	if(e)
	{
		...
		if(request->query || request->saml_artifacts || request->saml_asserion_id_ref)
		{
			...
			if(request->query)
			{
				axiom_node_t *n1 = saml_query_to_om(request->query, n, env);
				t = axiom_node_to_string(n1, env);
			}
			if(request->saml_artifacts)
			{
				size = axutil_array_list_size(request->saml_artifacts, env);
...
AXIS2_EXTERN axis2_char_t* AXIS2_CALL saml_request_get_id(saml_request_t *request, axutil_env_t *env)
{
	if(request)
	{
...
AXIS2_EXTERN int AXIS2_CALL saml_request_set_minor_version(saml_request_t *request, int version, axutil_env_t *env)
{
	if(request->minor_version)
	{
		AXIS2_FREE(env->allocator, request->minor_version);
...
AXIS2_EXTERN int AXIS2_CALL saml_request_set_major_version(saml_request_t *request, int version, axutil_env_t *env)
{
	if(request->major_version)
	{
		AXIS2_FREE(env->allocator, request->major_version);
...
AXIS2_EXTERN int AXIS2_CALL saml_request_set_issue_instant(saml_request_t *request, axutil_date_time_t *date_time, axutil_env_t *env)
{
	if(request->issue_instant)
	{
		axutil_date_time_free(request->issue_instant, env);
...
AXIS2_EXTERN  axutil_date_time_t* AXIS2_CALL saml_request_get_issue_instant(saml_request_t *request, axutil_env_t *env)
{
	if(request)
...
AXIS2_EXTERN int AXIS2_CALL saml_request_set_respond_withs(saml_request_t *request, axutil_array_list_t *responds, axutil_env_t *env)
{
	int size = 0, i = 0;
	axutil_qname_t *respond = NULL;
	if(request->saml_responds)
	{
		...
		for(i = 0; i < size; i++)
		{
			respond = axutil_array_list_get(request->saml_responds, env, i);
			if(respond)
				axutil_qname_free(respond, env);
		}
		axutil_array_list_free(request->saml_responds, env);
...
AXIS2_EXTERN  axutil_array_list_t* AXIS2_CALL saml_request_get_respond_withs(saml_request_t *request, axutil_env_t *env)
{
	if(request)
...
AXIS2_EXTERN int AXIS2_CALL saml_request_add_respond_with(saml_request_t *request, axutil_qname_t *respond, axutil_env_t *env)
{
	if(!request->saml_responds)
	{
		request->saml_responds = axutil_array_list_create(env, SAML_ARRAY_LIST_DEF);
...
AXIS2_EXTERN int AXIS2_CALL saml_request_remove_respond_with(saml_request_t *request, int index, axutil_env_t *env)
{
...
AXIS2_EXTERN int AXIS2_CALL saml_request_set_query(saml_request_t *request, saml_query_t *query, axutil_env_t *env)
{
	if(request->query)
	{
		saml_query_free(request->query, env);
...
AXIS2_EXTERN saml_query_t* AXIS2_CALL saml_request_get_query(saml_request_t *request, axutil_env_t *env)
{
	if(request)
...
AXIS2_EXTERN int AXIS2_CALL saml_request_set_id_refs(saml_request_t *request, axutil_array_list_t *id_refs, axutil_env_t *env)
{
	int size = 0, i = 0;
	if(request->saml_asserion_id_ref)
	{
...
AXIS2_EXTERN axutil_array_list_t* AXIS2_CALL saml_request_get_id_refs(saml_request_t *request, axutil_env_t *env)
{
	if(request)
...
AXIS2_EXTERN int AXIS2_CALL saml_request_add_id_refs(saml_request_t *request, axis2_char_t *id_reference, axutil_env_t *env)
{
	if(!request->saml_asserion_id_ref)
	{
		request->saml_asserion_id_ref = axutil_array_list_create(env, SAML_ARRAY_LIST_DEF);
...
AXIS2_EXTERN int AXIS2_CALL saml_request_remove_id_refs(saml_request_t *request, int index, axutil_env_t *env)
{
	axis2_char_t *id_ref;
	if(request->saml_asserion_id_ref)
	{
...
AXIS2_EXTERN int AXIS2_CALL saml_request_set_artifacts(saml_request_t *request, axutil_array_list_t *artifacts, axutil_env_t *env)
{
	int size = 0, i = 0;
	saml_artifact_t *artifact = NULL;
	if(request->saml_artifacts)
...
AXIS2_EXTERN axutil_array_list_t*  AXIS2_CALL saml_request_get_artifacts(saml_request_t *request, axutil_env_t *env)
{
	if(request)
...
AXIS2_EXTERN int AXIS2_CALL saml_request_add_artifact(saml_request_t *request, saml_artifact_t *artifact, axutil_env_t *env)
{
	if(!request->saml_artifacts)
	{
		request->saml_artifacts = axutil_array_list_create(env, SAML_ARRAY_LIST_DEF);
...
AXIS2_EXTERN int AXIS2_CALL saml_request_remove_artifact(saml_request_t *request, int index, axutil_env_t *env)
{
	saml_artifact_t *ar;
	if(request->saml_artifacts)
	{
...
AXIS2_EXTERN axis2_bool_t AXIS2_CALL saml_request_check_validity(saml_request_t *request, axutil_env_t *env)
{
	if(request->query)
		return AXIS2_TRUE;
...
AXIS2_EXTERN int AXIS2_CALL saml_request_set_signature(saml_request_t *request, axutil_env_t *env, oxs_sign_ctx_t *sig_ctx)
{
	if(request->sig_ctx)
	{
		oxs_sign_ctx_free(request->sig_ctx, env);
...
AXIS2_EXTERN int AXIS2_CALL saml_request_unset_signature(saml_request_t *request, axutil_env_t *env)
{
	if(request->sig_ctx)
	{
		oxs_sign_ctx_free(request->sig_ctx, env);
...
AXIS2_EXTERN void AXIS2_CALL saml_request_set_default_signature(saml_request_t *request, axutil_env_t *env, oxs_sign_ctx_t *sig_ctx)
{
	if(request->sig_ctx)
	{
		oxs_sign_ctx_free(request->sig_ctx, env);
